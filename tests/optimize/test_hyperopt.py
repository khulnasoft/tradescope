...
def test_start_not_installed(mocker, default_conf, import_fails) -> None:
    ...
    mocker.patch('tradescope.optimize.hyperopt.Hyperopt.start', start_mock)
...
def test_start_no_data(mocker, hyperopt_conf, tmp_path) -> None:
    ...
    patched_configuration_load_config_file(mocker, hyperopt_conf)
    mocker.patch('tradescope.data.history.load_pair_history', MagicMock(return_value=pd.DataFrame))
    mocker.patch(
        'tradescope.optimize.hyperopt.get_timerange',
...
def test_start_filelock(mocker, hyperopt_conf, caplog) -> None:
    ...
    patched_configuration_load_config_file(mocker, hyperopt_conf)
    mocker.patch('tradescope.optimize.hyperopt.Hyperopt.__init__', hyperopt_mock)
    patch_exchange(mocker)
    ...
    pargs = get_args(args)
    start_hyperopt(pargs)
    assert log_has("Another running instance of tradescope Hyperopt detected.", caplog)
...
def test_start_calls_optimizer(mocker, hyperopt_conf, capsys) -> None:
    dumper = mocker.patch('tradescope.optimize.hyperopt.dump')
    dumper2 = mocker.patch('tradescope.optimize.hyperopt.Hyperopt._save_result')
    mocker.patch('tradescope.optimize.hyperopt.calculate_market_change', return_value=1.5)
    mocker.patch('tradescope.optimize.hyperopt.file_dump_json')

    mocker.patch('tradescope.optimize.backtesting.Backtesting.load_bt_data',
    ...
    mocker.patch('tradescope.optimize.hyperopt.INITIAL_POINTS', 2)

    parallel = mocker.patch(
        'tradescope.optimize.hyperopt.Hyperopt.run_optimizer_parallel',
...
def test_generate_optimizer(mocker, hyperopt_conf) -> None:
    ...
    mocker.patch('tradescope.optimize.hyperopt.Backtesting.backtest', return_value=backtest_result)
    mocker.patch('tradescope.optimize.hyperopt.get_timerange',
                 return_value=(dt_utc(2017, 12, 10), dt_utc(2017, 12, 13)))
    patch_exchange(mocker)
    mocker.patch.object(Path, 'open')
    mocker.patch('tradescope.configuration.config_validation.validate_config_schema')
    mocker.patch('tradescope.optimize.hyperopt.load', return_value={'XRP/BTC': None})
...
def test_clean_hyperopt(mocker, hyperopt_conf, caplog):
    ...
    mocker.patch("tradescope.strategy.hyper.HyperStrategyMixin.load_params_from_file",
                 MagicMock(return_value={}))
    mocker.patch("tradescope.optimize.hyperopt.Path.is_file", MagicMock(return_value=True))
    unlinkmock = mocker.patch("tradescope.optimize.hyperopt.Path.unlink", MagicMock())
...
def test_simplified_interface_all_failed(mocker, hyperopt_conf, caplog) -> None:
    mocker.patch('tradescope.optimize.hyperopt.dump', MagicMock())
    mocker.patch('tradescope.optimize.hyperopt.file_dump_json')
    mocker.patch('tradescope.optimize.backtesting.Backtesting.load_bt_data',
                 MagicMock(return_value=(MagicMock(), None)))
    mocker.patch(
    ...
    mocker.patch('tradescope.optimize.hyperopt_auto.HyperOptAuto._generate_indicator_space',
                 return_value=[])
...
def test_in_strategy_auto_hyperopt_per_epoch(mocker, hyperopt_conf, tmp_path, fee) -> None:
    ...
    go = mocker.patch('tradescope.optimize.hyperopt.Hyperopt.generate_optimizer',
                      return_value={
                            'loss': 0.05,
...
def test_stake_amount_unlimited_max_open_trades(mocker, hyperopt_conf, tmp_path, fee) -> None:
    ...
    hyperopt = Hyperopt(hyperopt_conf)
    mocker.patch('tradescope.optimize.hyperopt.Hyperopt._get_params_dict',
                 return_value={
                     'max_open_trades': -1
                     })
