...
from tradescope.loggers import setup_logging, setup_logging_pre
...
from tradescope.rpc.api_server.api_auth import create_token, get_user_from_token
...
def test_api_ui_fallback(botclient, mocker):
    ...
    assert rc.status_code == 200
    assert '`tradescope install-ui`' in rc.text
    ...
    assert '`tradescope install-ui`' in rc.text or '<!DOCTYPE html>' in rc.text
...
def test_api_auth():
    with pytest.raises(ValueError):
        create_token({'identity': {'u': 'Tradescope'}}, 'secret1234', token_type="NotATokenType")

    token = create_token({'identity': {'u': 'Tradescope'}}, 'secret1234')
...
def test_api__init__(default_conf, mocker):
    ...
    mocker.patch('tradescope.rpc.telegram.Telegram._init')
    mocker.patch('tradescope.rpc.api_server.webserver.ApiServer.start_api', MagicMock())
    apiserver = ApiServer(default_conf)
    apiserver.add_rpc_handler(RPC(get_patched_tradescopebot(mocker, default_conf)))
...
def test_api_UvicornServer(mocker):
    thread_mock = mocker.patch('tradescope.rpc.api_server.uvicorn_threaded.threading.Thread')
...
def test_api_UvicornServer_run(mocker):
    serve_mock = mocker.patch('tradescope.rpc.api_server.uvicorn_threaded.UvicornServer.serve',
                              get_mock_coro(None))
    s = UvicornServer(uvicorn.Config(MagicMock(), port=8080, host='127.0.0.1'))
...
def test_api_run(default_conf, mocker, caplog):
    ...
    mocker.patch('tradescope.rpc.api_server.webserver.UvicornServer', server_mock)
    ...
    caplog.clear()
    mocker.patch('tradescope.rpc.api_server.webserver.UvicornServer',
...
def test_api_show_config(botclient):
    ...
    assert response['bot_name'] == 'tradescope'
...
def test_api_plot_config(botclient, mocker):
    ...
    mocker.patch('tradescope.rpc.api_server.api_v1.get_rpc_optional', return_value=None)
...
def test_api_pairlists_evaluate(botclient, tmp_path, mocker):
    ...
    plm = mocker.patch('tradescope.rpc.api_server.api_background_tasks.__run_pairlist',
                       return_value=None)
    body = {
        "pairlists": [
            {"method": "StaticPairList", },
...
def test_api_backtesting(botclient, mocker, fee, caplog, tmp_path):
    try:
        ...
        mocker.patch('tradescope.optimize.backtesting.Backtesting.backtest_one_strategy',
...
def test_api_ws_send_msg(default_conf, mocker, caplog):
    try:
        ...
        mocker.patch('tradescope.rpc.telegram.Telegram._init')
        mocker.patch('tradescope.rpc.api_server.ApiServer.start_api')
        apiserver = ApiServer(default_conf)
        apiserver.add_rpc_handler(RPC(get_patched_tradescopebot(mocker, default_conf)))
