...
def patch_RPCManager(mocker) -> MagicMock:
    ...
    rpc_mock = mocker.patch('tradescope.tradescopebot.RPCManager.send_msg', MagicMock())
    return rpc_mock
...
def test_tradescopebot_state(mocker, default_conf_usdt, markets) -> None:
    mocker.patch(f'{EXMS}.markets', PropertyMock(return_value=markets))
    tradescope = get_patched_tradescopebot(mocker, default_conf_usdt)
    assert tradescope.state is State.RUNNING

    default_conf_usdt.pop('initial_state')
    tradescope = TradescopeBot(default_conf_usdt)
    assert tradescope.state is State.STOPPED


def test_process_stopped(mocker, default_conf_usdt) -> None:

    tradescope = get_patched_tradescopebot(mocker, default_conf_usdt)
    coo_mock = mocker.patch('tradescope.tradescopebot.TradescopeBot.cancel_all_open_orders')
    tradescope.process_stopped()
...
def test_process_calls_sendmsg(mocker, default_conf_usdt) -> None:
    tradescope = get_patched_tradescopebot(mocker, default_conf_usdt)
    tradescope.process()
    assert tradescope.rpc.process_msg_queue.call_count == 1


def test_bot_cleanup(mocker, default_conf_usdt, caplog) -> None:
    mock_cleanup = mocker.patch('tradescope.tradescopebot.Trade.commit')
    coo_mock = mocker.patch('tradescope.tradescopebot.TradescopeBot.cancel_all_open_orders')
    tradescope = get_patched_tradescopebot(mocker, default_conf_usdt)
    tradescope.cleanup()
    ...
    tradescope.config['cancel_open_orders_on_exit'] = True
    tradescope.cleanup()
    assert coo_mock.call_count == 1


def test_bot_cleanup_db_errors(mocker, default_conf_usdt, caplog) -> None:
    mocker.patch('tradescope.tradescopebot.Trade.commit',
                 side_effect=OperationalException())
    mocker.patch('tradescope.tradescopebot.TradescopeBot.check_for_open_trades',
                 side_effect=OperationalException())
    tradescope = get_patched_tradescopebot(mocker, default_conf_usdt)
    tradescope.emc = MagicMock()
    tradescope.emc.shutdown = MagicMock()
    tradescope.cleanup()
    assert tradescope.emc.shutdown.call_count == 1
...
def test_order_dict(default_conf_usdt, mocker, runmode, caplog) -> None:
    ...
    tradescope = TradescopeBot(conf)
    if runmode == RunMode.LIVE:
        assert not log_has_re(r".*stoploss_on_exchange .* dry-run", caplog)
    assert tradescope.strategy.order_types['stoploss_on_exchange']
    ...
    tradescope = TradescopeBot(conf)
    assert not tradescope.strategy.order_types['stoploss_on_exchange']
    assert not log_has_re(r".*stoploss_on_exchange .* dry-run", caplog)
...
def test_get_trade_stake_amount(default_conf_usdt, mocker) -> None:
    ...
    result = tradescope.wallets.get_trade_stake_amount('ETH/USDT', 1)
...
def test_load_strategy_no_keys(default_conf_usdt, mocker, runmode, caplog) -> None:
    ...
    conf['runmode'] = runmode
    erm = mocker.patch('tradescope.tradescopebot.ExchangeResolver.load_exchange')

    tradescope = TradescopeBot(conf)
    strategy_config = tradescope.strategy.config
    ...
    for i in range(0, max_open):
        ...
        if expected[i] is not None:
            ...
            with pytest.raises(DependencyException):
                tradescope.wallets.get_trade_stake_amount('ETH/USDT', 1)
...
def test_edge_called_in_process(mocker, edge_conf) -> None:
    ...
    patch_exchange(mocker)
    tradescope = TradescopeBot(edge_conf)
    patch_get_signal(tradescope)
    tradescope.process()
    assert tradescope.active_pair_whitelist == ['NEO/BTC', 'LTC/BTC']
...
def test_edge_overrides_stake_amount(mocker, edge_conf) -> None:
    ...
    assert tradescope.wallets.get_trade_stake_amount(
        'NEO/BTC', 1, tradescope.edge) == (999.9 * 0.5 * 0.01) / 0.20
    assert tradescope.wallets.get_trade_stake_amount(
        'LTC/BTC', 1, tradescope.edge) == (999.9 * 0.5 * 0.01) / 0.21
...
def test_edge_overrides_stoploss(limit_order, fee, caplog, mocker,
    ...
    tradescope = TradescopeBot(edge_conf)
    tradescope.active_pair_whitelist = ['NEO/BTC']
    patch_get_signal(tradescope)
    ...
    assert tradescope.handle_trade(trade) is not ignore_strat_sl
...
def test_create_trade(default_conf_usdt, ticker_usdt, limit_order,
    ...
    tradescope = TradescopeBot(default_conf_usdt)
    patch_get_signal(tradescope, enter_short=is_short, enter_long=not is_short)
    tradescope.create_trade('ETH/USDT')
...
def test_create_trade_minimal_amount(
    ...
    tradescope = TradescopeBot(default_conf_usdt)
    tradescope.config['stake_amount'] = stake_amount
    patch_get_signal(tradescope, enter_short=is_short, enter_long=not is_short)

    if create:
        assert tradescope.create_trade('ETH/USDT')
        if amount_enough:
        ...
        assert not tradescope.create_trade('ETH/USDT')
        if not max_open_trades:
            assert tradescope.wallets.get_trade_stake_amount(
                'ETH/USDT', default_conf_usdt['max_open_trades'], tradescope.edge) == 0
...
def test_enter_positions_no_pairs_left(default_conf_usdt, ticker_usdt, limit_buy_order_usdt_open,
    ...
    mocker.patch('tradescope.configuration.config_validation._validate_whitelist')
    default_conf_usdt['exchange']['pair_whitelist'] = whitelist
    tradescope = TradescopeBot(default_conf_usdt)
    patch_get_signal(tradescope)

    n = tradescope.enter_positions()
    assert n == positions
    if positions:
...
def test_handle_protections(mocker, default_conf_usdt, fee, is_short):
    ...
    tradescope = get_patched_tradescopebot(mocker, default_conf_usdt)
    tradescope.protections._protection_handlers[1].global_stop = MagicMock(
        return_value=ProtectionReturn(True, dt_now() + timedelta(hours=1), "asdf"))
    create_mock_trades(fee, is_short)
    tradescope.handle_protections('ETC/BTC', '*')
    send_msg_mock = tradescope.rpc.send_msg
...
def test_create_trade_no_signal(default_conf_usdt, fee, mocker) -> None:
    ...
    tradescope = TradescopeBot(default_conf_usdt)
    patch_get_signal(tradescope, enter_long=False, exit_long=False)
...
def test_create_trades_preopen(default_conf_usdt, ticker_usdt, fee, mocker,
    ...
    tradescope.execute_entry('ETH/USDT', default_conf_usdt['stake_amount'])
    tradescope.execute_entry('NEO/BTC', default_conf_usdt['stake_amount'])
    ...
    assert tradescope.create_trade('ETH/USDT')
    assert tradescope.create_trade('NEO/BTC')
...
def test_process_exchange_failures(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    patch_get_signal(worker.tradescope)
...
def test_process_operational_exception(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    assert worker.tradescope.state == State.RUNNING

    worker._process_running()
    assert worker.tradescope.state == State.STOPPED
...
def test_process_trade_no_whitelist_pair(default_conf_usdt, ticker_usdt, limit_buy_order_usdt,
    ...
    assert pair not in tradescope.active_pair_whitelist
    tradescope.process()
    assert pair in tradescope.active_pair_whitelist
    # Make sure each pair is only in the list once
    assert len(tradescope.active_pair_whitelist) == len(set(tradescope.active_pair_whitelist))
...
def test_process_informative_pairs_added(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    tradescope = TradescopeBot(default_conf_usdt)
    tradescope.strategy.informative_pairs = inf_pairs
    # patch_get_signal(tradescope)
...
def test_execute_entry(mocker, default_conf_usdt, fee, limit_order,
    ...
    tradescope = TradescopeBot(default_conf_usdt)
    tradescope.strategy.confirm_trade_entry = MagicMock(return_value=False)
    tradescope.strategy.leverage = MagicMock(return_value=leverage)
    stake_amount = 2
    bid = 0.11
    ...
    mocker.patch.multiple(
        'tradescope.exchange.okx.Okx',
        get_max_pair_stake_amount=MagicMock(return_value=500000),
    )
    pair = 'ETH/USDT'

    assert not tradescope.execute_entry(pair, stake_amount, is_short=is_short)
    assert enter_rate_mock.call_count == 1
    assert enter_mm.call_count == 0
    assert tradescope.strategy.confirm_trade_entry.call_count == 1
    enter_rate_mock.reset_mock()

    open_order['id'] = '22'
    tradescope.strategy.confirm_trade_entry = MagicMock(return_value=True)
    assert tradescope.execute_entry(pair, stake_amount)
    ...
    open_order['id'] = '33'
    fix_price = 0.06
    assert tradescope.execute_entry(pair, stake_amount, fix_price, is_short=is_short)
    ...
    assert tradescope.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    tradescope.strategy.custom_stake_amount = lambda **kwargs: 150.0
    assert tradescope.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    # Exception case
    order['id'] = '557'
    tradescope.strategy.custom_stake_amount = lambda **kwargs: 20 / 0
    assert tradescope.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    order['id'] = '66'
    mocker.patch(f'{EXMS}.create_order', MagicMock(return_value=order))
    assert not tradescope.execute_entry(pair, stake_amount)
    assert tradescope.strategy.leverage.call_count == 0 if trading_mode == 'spot' else 2
    ...
    with pytest.raises(PricingError, match="Could not determine entry price."):
        tradescope.execute_entry(pair, stake_amount, is_short=is_short)

    # In case of custom entry price
    mocker.patch(f'{EXMS}.get_rate', return_value=0.50)
    order['status'] = 'open'
    order['id'] = '5566'
    tradescope.strategy.custom_entry_price = lambda **kwargs: 0.508
    assert tradescope.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    order['status'] = 'open'
    order['id'] = '5567'
    tradescope.strategy.custom_entry_price = lambda **kwargs: None
    ...
    order['status'] = 'open'
    order['id'] = '5568'
    tradescope.strategy.custom_entry_price = lambda **kwargs: "string price"
    assert tradescope.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    tradescope.exchange.get_max_pair_stake_amount = MagicMock(return_value=500)

    assert tradescope.execute_entry(pair, 2000, is_short=is_short)
    ...
    tradescope.strategy.leverage.reset_mock()
    assert tradescope.execute_entry(pair, 200, leverage_=3)
    assert tradescope.strategy.leverage.call_count == 0
...
def test_execute_entry_confirm_error(mocker, default_conf_usdt, fee, limit_order, is_short) -> None:
    ...
    tradescope.strategy.confirm_trade_entry = MagicMock(side_effect=ValueError)
    assert tradescope.execute_entry(pair, stake_amount)

    limit_order[entry_side(is_short)]['id'] = '222'
    tradescope.strategy.confirm_trade_entry = MagicMock(side_effect=Exception)
    assert tradescope.execute_entry(pair, stake_amount)
...
def test_execute_entry_min_leverage(mocker, default_conf_usdt, fee, limit_order, is_short) -> None:
    ...
    stake_amount = 2
    pair = 'SOL/BUSD:BUSD'
    tradescope.strategy.leverage = MagicMock(return_value=5.0)
...
def test_enter_positions(mocker, default_conf_usdt, return_value, side_effect,
    ...
    mock_ct = mocker.patch(
        'tradescope.tradescopebot.TradescopeBot.create_trade',
        MagicMock(
...
def test_exit_positions(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    ...
    mocker.patch('tradescope.tradescopebot.TradescopeBot.handle_trade', MagicMock(return_value=True))
    ...
    n = tradescope.exit_positions(trades)
    assert n == 0
    # Test amount not modified by fee-logic
    assert not log_has_re(r'Applying fee to amount for Trade .*', caplog)

    gra = mocker.patch('tradescope.tradescopebot.TradescopeBot.get_real_amount', return_value=0.0)
...
def test_exit_positions_exception(mocker, default_conf_usdt, limit_order, caplog, is_short) -> None:
    ...
    mocker.patch(
        'tradescope.tradescopebot.TradescopeBot.handle_trade',
...
def test_update_trade_state(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    ...
    mocker.patch('tradescope.tradescopebot.TradescopeBot.handle_trade', MagicMock(return_value=True))
    mocker.patch('tradescope.tradescopebot.TradescopeBot._notify_enter')
    mocker.patch(f'{EXMS}.fetch_order', return_value=order)
    mocker.patch(f'{EXMS}.get_trades_for_order', return_value=[])
    mocker.patch('tradescope.tradescopebot.TradescopeBot.get_real_amount', return_value=0.0)
    order_id = order['id']
    ...
    tradescope.strategy.order_filled = MagicMock(return_value=None)
    assert not tradescope.update_trade_state(trade, None)
    assert log_has_re(r'Orderid for trade .* is empty.', caplog)
    caplog.clear()
    # Add datetime explicitly since sqlalchemy defaults apply only once written to database
    tradescope.update_trade_state(trade, order_id)
    # Test amount not modified by fee-logic
    assert not log_has_re(r'Applying fee to .*', caplog)
    caplog.clear()
    assert not trade.has_open_orders
    assert trade.amount == order['amount']
    assert tradescope.strategy.order_filled.call_count == 1

    mocker.patch('tradescope.tradescopebot.TradescopeBot.get_real_amount', return_value=0.01)
    assert trade.amount == 30.0
    # test amount modified by fee-logic
    tradescope.update_trade_state(trade, order_id)
    ...
    tradescope.strategy.order_filled = MagicMock(return_value=None)
    mocker.patch('tradescope.tradescopebot.TradescopeBot.get_real_amount', side_effect=ValueError)
    mocker.patch(f'{EXMS}.fetch_order', return_value=limit_buy_order_usdt_new)
    res = tradescope.update_trade_state(trade, order_id)
    # Cancelled empty
    assert res is True
    assert tradescope.strategy.order_filled.call_count == 0
...
def test_update_trade_state_withorderdict(
    ...
    tradescope.update_trade_state(trade, order_id, order)
...
def test_update_trade_state_exception(mocker, default_conf_usdt, is_short, limit_order,
    ...
    mocker.patch(
        'tradescope.tradescopebot.TradescopeBot.get_real_amount',
        side_effect=DependencyException()
    )
    tradescope.update_trade_state(trade, open_order_id)
...
def test_update_trade_state_orderexception(mocker, default_conf_usdt, caplog) -> None:
    ...
    # Test raise of OperationalException exception
    grm_mock = mocker.patch("tradescope.tradescopebot.TradescopeBot.get_real_amount", MagicMock())
    tradescope.update_trade_state(trade, open_order_id)
...
def test_update_trade_state_sell(
    ...
    mocker.patch('tradescope.wallets.Wallets.update', wallet_mock)
    ...
    tradescope.update_trade_state(trade, trade.open_orders_ids[-1], l_order)
...
def test_handle_trade(
    ...
    patch_get_signal(tradescope, enter_long=False, exit_short=is_short,
                     exit_long=not is_short, exit_tag='sell_signal1')
    assert tradescope.handle_trade(trade) is True
...
def test_handle_overlapping_signals(
    ...
    tradescope = TradescopeBot(default_conf_usdt)
    if is_short:
        patch_get_signal(tradescope, enter_long=False, enter_short=True, exit_short=True)
    else:
        patch_get_signal(tradescope, enter_long=True, exit_long=True)
    tradescope.strategy.min_roi_reached = MagicMock(return_value=False)
    ...
    patch_get_signal(tradescope, enter_long=False)
    assert tradescope.handle_trade(trades[0]) is False
    trades = Trade.session.scalars(select(Trade)).all()
    for trade in trades:
        trade.is_short = is_short
    nb_trades = len(trades)
    ...
    if is_short:
        patch_get_signal(tradescope, enter_long=False, exit_short=True)
    else:
        patch_get_signal(tradescope, enter_long=False, exit_long=True)
    trades = Trade.session.scalars(select(Trade)).all()
    for trade in trades:
        trade.is_short = is_short
    assert tradescope.handle_trade(trades[0]) is True
...
def test_handle_trade_roi(default_conf_usdt, ticker_usdt, limit_order_open, fee, mocker, caplog,
    ...
    tradescope = get_patched_tradescopebot(mocker, default_conf_usdt)
    patch_get_signal(tradescope, enter_short=is_short, enter_long=not is_short)
    tradescope.strategy.min_roi_reached = MagicMock(return_value=True)
    ...
    caplog.clear()
    patch_get_signal(tradescope)
    assert tradescope.handle_trade(trade)
...
def test_handle_trade_use_exit_signal(
    ...
    patch_get_signal(tradescope, enter_long=False, exit_long=False)
    assert not tradescope.handle_trade(trade)
...
def test_manage_open_orders_entry_usercustom(
    ...
    tradescope.strategy.check_entry_timeout = MagicMock(return_value=False)
    tradescope.manage_open_orders()
    assert cancel_order_mock.call_count == 0
    ...
    nb_trades = len(trades)
    assert nb_trades == 1
    assert tradescope.strategy.check_entry_timeout.call_count == 1
    tradescope.strategy.check_entry_timeout = MagicMock(side_effect=KeyError)
    ...
    assert nb_trades == 1
    assert tradescope.strategy.check_entry_timeout.call_count == 1
    tradescope.strategy.check_entry_timeout = MagicMock(return_value=True)
...
def test_manage_open_orders_entry(
    ...
    tradescope.strategy.check_entry_timeout = MagicMock(return_value=False)
    tradescope.strategy.adjust_entry_price = MagicMock(return_value=1234)
    ...
    assert tradescope.strategy.check_entry_timeout.call_count == 0
    # Entry adjustment is never called
    assert tradescope.strategy.adjust_entry_price.call_count == 0
...
def test_adjust_entry_cancel(
    ...
    assert tradescope.strategy.adjust_entry_price.call_count == 1
...
def test_adjust_entry_replace_fail(
    ...
    mocker.patch('tradescope.tradescopebot.sleep')
...
def test_adjust_entry_replace_fail_create_order(
    ...
    mocker.patch('tradescope.tradescopebot.sleep')
    mocker.patch('tradescope.tradescopebot.TradescopeBot.execute_entry',
                 side_effect=DependencyException())
...
def test_adjust_entry_maintain_replace(
    ...
    tradescope.get_valid_enter_price_and_stake = MagicMock(return_value={100, 10, 1})
    tradescope.strategy.adjust_entry_price = MagicMock(return_value=1234)
...
def test_manage_open_orders_exit_usercustom(
    ...
    et_mock = mocker.patch('tradescope.tradescopebot.TradescopeBot.execute_trade_exit')
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
    ...
    tradescope.strategy.check_exit_timeout = MagicMock(return_value=False)
    tradescope.strategy.check_entry_timeout = MagicMock(return_value=False)
    # Return false - No impact
    tradescope.manage_open_orders()
    assert cancel_order_mock.call_count == 0
    assert rpc_mock.call_count == 1
    assert tradescope.strategy.check_exit_timeout.call_count == 1
    assert tradescope.strategy.check_entry_timeout.call_count == 0

    tradescope.strategy.check_exit_timeout = MagicMock(side_effect=KeyError)
    tradescope.strategy.check_entry_timeout = MagicMock(side_effect=KeyError)
    ...
    # Return True - sells!
    tradescope.strategy.check_exit_timeout = MagicMock(return_value=True)
    tradescope.strategy.check_entry_timeout = MagicMock(return_value=True)
    tradescope.manage_open_orders()
    assert cancel_order_mock.call_count == 1
    assert rpc_mock.call_count == 2
    ...
    mocker.patch('tradescope.persistence.Trade.get_canceled_exit_order_count', return_value=1)
    mocker.patch('tradescope.tradescopebot.TradescopeBot.execute_trade_exit',
                 side_effect=DependencyException)
    tradescope.manage_open_orders()
    ...
    with patch('tradescope.tradescopebot.TradescopeBot.handle_cancel_exit', return_value=False):
        tradescope.manage_open_orders()
...
def test_manage_open_orders_exit(
    ...
    assert tradescope.strategy.check_exit_timeout.call_count == 0
    assert tradescope.strategy.check_entry_timeout.call_count == 0
...
def test_manage_open_orders_partial_fee(
    ...
    mocker.patch('tradescope.wallets.Wallets.get_free', MagicMock(return_value=0))
    patch_exchange(mocker)
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
...
def test_manage_open_orders_partial_except(
    ...
    mocker.patch('tradescope.tradescopebot.TradescopeBot.get_real_amount',
...
def test_manage_open_orders_exception(default_conf_usdt, ticker_usdt, open_trade_usdt, mocker,
    ...
    mocker.patch.multiple(
        'tradescope.tradescopebot.TradescopeBot',
...
def test_handle_cancel_enter(mocker, caplog, default_conf_usdt, limit_order, is_short, fee) -> None:
    ...
    tradescope = TradescopeBot(default_conf_usdt)
    tradescope._notify_enter_cancel = MagicMock()
    ...
    assert tradescope.handle_cancel_enter(trade, l_order, trade.open_orders[0], reason)
    assert cancel_order_mock.call_count == 1

    cancel_order_mock.reset_mock()
    caplog.clear()
    l_order['filled'] = 0.01
    assert not tradescope.handle_cancel_enter(trade, l_order, trade.open_orders[0], reason)
    assert cancel_order_mock.call_count == 0
    ...
    assert not tradescope.handle_cancel_enter(
...
def test_handle_cancel_enter_exchanges(mocker, caplog, default_conf_usdt, is_short, fee,
    ...
    notify_mock = mocker.patch('tradescope.tradescopebot.TradescopeBot._notify_enter_cancel')
    tradescope = TradescopeBot(default_conf_usdt)
    ...
    Trade.session.add(trade)
    Trade.commit()
    assert tradescope.handle_cancel_enter(
...
def test_handle_cancel_exit_limit(mocker, default_conf_usdt, fee, is_short,
    ...
    mocker.patch('tradescope.tradescopebot.TradescopeBot.handle_order_fee')
    ...
    assert tradescope.handle_cancel_exit(trade, order, order_obj, reason)
    assert cancel_order_mock.call_count == 1
    ...
    assert not tradescope.handle_cancel_exit(trade, order, order_obj, reason)
...
def test_handle_cancel_exit_cancel_exception(mocker, default_conf_usdt) -> None:
    ...
    # assert not tradescope.handle_cancel_exit(trade, order, reason)
...
def test_execute_trade_exit_up(default_conf_usdt, ticker_usdt, fee, ticker_usdt_sell_up, mocker,
    ...
    tradescope = TradescopeBot(default_conf_usdt)
    patch_get_signal(tradescope, enter_short=is_short, enter_long=not is_short)
    tradescope.strategy.confirm_trade_exit = MagicMock(return_value=False)
    ...
    assert trade
    assert tradescope.strategy.confirm_trade_exit.call_count == 0
    ...
    assert tradescope.strategy.confirm_trade_exit.call_count == 1
    assert id(tradescope.strategy.confirm_trade_exit.call_args_list[0][1]['trade']) != id(trade)
    assert tradescope.strategy.confirm_trade_exit.call_args_list[0][1]['trade'].id == trade.id

    # Repatch with true
    tradescope.strategy.confirm_trade_exit = MagicMock(return_value=True)
    tradescope.execute_trade_exit(
        trade=trade,
        limit=(ticker_usdt_sell_down()['ask'] if is_short else ticker_usdt_sell_up()['bid']),
        exit_check=ExitCheckTuple(exit_type=ExitType.ROI)
...
def test_execute_trade_exit_custom_exit_price(
    ...
    tradescope = TradescopeBot(config)
    patch_get_signal(tradescope, enter_short=is_short, enter_long=not is_short)
    tradescope.strategy.confirm_trade_exit = MagicMock(return_value=False)
    ...
    tradescope.strategy.custom_exit_price = lambda **kwargs: 2.25
    tradescope.execute_trade_exit(
        trade=trade,
...
def test_execute_trade_exit_market_order(
    ...
    tradescope.config['order_types']['exit'] = 'market'
...
def test_execute_trade_exit_insufficient_funds_error(default_conf_usdt, ticker_usdt, fee, is_short,
    ...
    tradescope = get_patched_tradescopebot(mocker, default_conf_usdt)
    mock_insuf = mocker.patch('tradescope.tradescopebot.TradescopeBot.handle_insufficient_funds')
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
        get_fee=fee,
    ...
    assert not tradescope.execute_trade_exit(
        trade=trade,
        limit=ticker_usdt_sell_up()['ask' if is_short else 'bid'],
...
def test_exit_profit_only(
    ...
    tradescope = TradescopeBot(default_conf_usdt)
    patch_get_signal(tradescope, enter_short=is_short, enter_long=not is_short)
    tradescope.strategy.custom_exit = MagicMock(return_value=None)
    ...
    tradescope.wallets.update()
    if profit_only:
        assert tradescope.handle_trade(trade) is False
        # Custom-exit is called
        tradescope.strategy.custom_exit.call_count == 1

    patch_get_signal(tradescope, enter_long=False, exit_short=is_short, exit_long=not is_short)
    assert tradescope.handle_trade(trade) is handle_first

    if handle_second:
        tradescope.strategy.exit_profit_offset = 0.0
        assert tradescope.handle_trade(trade) is True
...
def test_sell_not_enough_balance(default_conf_usdt, limit_order, limit_order_open,
    ...
    trade.update_trade(oobj)
    patch_get_signal(tradescope, enter_long=False, exit_long=True)
    mocker.patch('tradescope.wallets.Wallets.get_free', MagicMock(return_value=trade.amount * 0.985))
...
def test__safe_exit_amount(default_conf_usdt, fee, caplog, mocker, amount_wallet, has_err):
    ...
    mocker.patch('tradescope.wallets.Wallets.get_free', MagicMock(return_value=amount_wallet))
    wallet_update = mocker.patch('tradescope.wallets.Wallets.update')
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
        exchange='binance',
    ...
    if has_err:
        with pytest.raises(DependencyException, match=r"Not enough amount to exit trade."):
            assert tradescope._safe_exit_amount(trade, trade.pair, trade.amount)
    else:
        wallet_update.reset_mock()
        assert trade.amount != amount_wallet
        assert tradescope._safe_exit_amount(trade, trade.pair, trade.amount) == amount_wallet
        assert log_has_re(r'.*Falling back to wallet-amount.', caplog)
        assert trade.amount == amount_wallet
        assert wallet_update.call_count == 1
        caplog.clear()
        wallet_update.reset_mock()
        assert tradescope._safe_exit_amount(trade, trade.pair, amount_wallet) == amount_wallet
...
def test_locked_pairs(default_conf_usdt, ticker_usdt, fee,
    ...
    assert tradescope.strategy.is_pair_locked(trade.pair, side='*')
    # Boths sides are locked
    assert tradescope.strategy.is_pair_locked(trade.pair, side='long')
    assert tradescope.strategy.is_pair_locked(trade.pair, side='short')
...
def test_ignore_roi_if_entry_signal(default_conf_usdt, limit_order, limit_order_open, is_short,
    ...
    if is_short:
        patch_get_signal(tradescope, enter_long=False, exit_short=False, exit_tag='something')
    else:
        patch_get_signal(tradescope, enter_long=False, exit_long=False, exit_tag='something')
    assert tradescope.handle_trade(trade) is True
...
def test_disable_ignore_roi_if_entry_signal(default_conf_usdt, limit_order, limit_order_open,
    ...
    patch_get_signal(tradescope, enter_long=not is_short, enter_short=is_short, exit_short=is_short)
    assert tradescope.handle_trade(trade) is True
...
def test_get_real_amount_quote(default_conf_usdt, trades_for_order, buy_order_fee, fee, caplog,
    ...
    assert tradescope.get_real_amount(trade, buy_order_fee, order_obj) == (amount * 0.001)
    assert log_has(
...
def test_get_real_amount_quote_dust(default_conf_usdt, trades_for_order, buy_order_fee, fee,
                                    caplog, mocker):
    mocker.patch(f'{EXMS}.get_trades_for_order', return_value=trades_for_order)
    walletmock = mocker.patch('tradescope.wallets.Wallets.update')
    mocker.patch('tradescope.wallets.Wallets.get_free', return_value=8.1122)
    amount = sum(x['amount'] for x in trades_for_order)
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
    ...
    assert tradescope.get_real_amount(trade, buy_order_fee, order_obj) is None
...
def test_get_real_amount(
    ...
    caplog.clear()
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, 'LTC/ETH', 'buy')
    res = tradescope.get_real_amount(trade, buy_order, order_obj)
...
def test_get_real_amount_multi(
    ...
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, 'LTC/ETH', 'buy')
    assert tradescope.get_real_amount(trade, buy_order_fee, order_obj) == expected_amount
    assert log_has(
        (
            'Applying fee on amount for Trade(id=None, pair=LTC/ETH, amount=8.00000000, '
...
def test_get_real_amount_invalid_order(default_conf_usdt, trades_for_order, buy_order_fee, fee,
    ...
    assert tradescope.get_real_amount(trade, limit_buy_order_usdt, order_obj) is None
...
def test_get_real_amount_fees_order(default_conf_usdt, market_buy_order_usdt_doublefee,
    ...
    assert tradescope.get_real_amount(trade, market_buy_order_usdt_doublefee, order_obj) is None
...
def test_get_real_amount_wrong_amount(default_conf_usdt, trades_for_order, buy_order_fee, fee,
    ...
    with pytest.raises(DependencyException, match=r"Half bought\? Amounts don't match"):
        tradescope.get_real_amount(trade, limit_buy_order_usdt, order_obj)
...
def test_get_real_amount_wrong_amount_rounding(default_conf_usdt, trades_for_order, buy_order_fee,
    ...
    assert pytest.approx(tradescope.get_real_amount(
...
def test_get_real_amount_open_trade_usdt(default_conf_usdt, fee, mocker):
    ...
    assert tradescope.get_real_amount(trade, order, order_obj) is None
...
def test_get_real_amount_in_point(default_conf_usdt, buy_order_fee, fee, mocker, caplog):
    ...
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, 'LTC/ETH', 'buy')
    res = tradescope.get_real_amount(trade, limit_buy_order_usdt, order_obj)
    assert res is None
    ...
    caplog.clear()
    tradescope.config['exchange']['unknown_fee_rate'] = 1
    res = tradescope.get_real_amount(trade, limit_buy_order_usdt, order_obj)
    assert res is None
...
def test_apply_fee_conditional(default_conf_usdt, fee, mocker, caplog,
    ...
    walletmock = mocker.patch('tradescope.wallets.Wallets.update')
    mocker.patch('tradescope.wallets.Wallets.get_free', return_value=wallet)
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
        exchange='binance',
    ...
    walletmock.reset_mock()
    # Amount is kept as is
    assert tradescope.apply_fee_conditional(trade, 'LTC', amount, fee_abs, order) == amount_exp
    assert walletmock.call_count == 1
...
def test_apply_fee_conditional_multibuy(default_conf_usdt, fee, mocker, caplog,
    ...
    assert tradescope.apply_fee_conditional(trade, 'LTC', amount, fee_abs, order1) == amount_exp
    assert walletmock.call_count == 1
    if fee_abs != 0 and amount_exp is None:
        assert log_has_re(r"Fee amount.*Eating.*dust\.", caplog)
...
def test_order_book_entry_pricing1(mocker, default_conf_usdt, order_book_l2, exception_thrown,
    ...
    if exception_thrown:
        with pytest.raises(PricingError):
            tradescope.exchange.get_rate('ETH/USDT', side="entry", is_short=False, refresh=True)
        assert log_has_re(
            r'ETH/USDT - Entry Price at location 1 from orderbook could not be determined.', caplog)
    else:
        assert tradescope.exchange.get_rate(
...
def test_check_depth_of_market(default_conf_usdt, mocker, order_book_l2) -> None:
    ...
    assert tradescope._check_depth_of_market('ETH/BTC', conf, side=SignalDirection.LONG) is False
...
def test_startup_state(default_conf_usdt, mocker):
    ...
    assert worker.tradescope.state is State.RUNNING
...
def test_sync_wallet_dry_run(mocker, default_conf_usdt, ticker_usdt, fee, limit_buy_order_usdt_open,
    ...
    bot = get_patched_tradescopebot(mocker, default_conf_usdt)
...
def test_cancel_all_open_orders(mocker, default_conf_usdt, fee, limit_order, limit_order_open,
    ...
    buy_mock = mocker.patch('tradescope.tradescopebot.TradescopeBot.handle_cancel_enter')
    sell_mock = mocker.patch('tradescope.tradescopebot.TradescopeBot.handle_cancel_exit')

    tradescope = get_patched_tradescopebot(mocker, default_conf_usdt)
    create_mock_trades(fee, is_short=is_short)
    trades = Trade.session.scalars(select(Trade)).all()
    assert len(trades) == MOCK_TRADE_COUNT
    tradescope.cancel_all_open_orders()
...
def test_check_for_open_trades(mocker, default_conf_usdt, fee, is_short):
    ...
    tradescope.check_for_open_trades()
    assert tradescope.rpc.send_msg.call_count == 0
    ...
    tradescope.check_for_open_trades()
    assert tradescope.rpc.send_msg.call_count == 1
    assert 'Handle these trades manually' in tradescope.rpc.send_msg.call_args[0][0]['status']
...
def test_startup_update_open_orders(mocker, default_conf_usdt, fee, caplog, is_short):
    ...
    tradescope.startup_update_open_orders()
    ...
    hto_mock = mocker.patch('tradescope.tradescopebot.TradescopeBot.handle_cancel_order')
...
def test_startup_backpopulate_precision(mocker, default_conf_usdt, fee, caplog):
    ...
    tradescope.startup_backpopulate_precision()
    trades = Trade.get_trades().all()
    for trade in trades:
...
def test_update_trades_without_assigned_fees(mocker, default_conf_usdt, fee, is_short):
    ...
    tradescope.update_trades_without_assigned_fees()
...
def test_reupdate_enter_order_fees(mocker, default_conf_usdt, fee, caplog, is_short):
    tradescope = get_patched_tradescopebot(mocker, default_conf_usdt)
    mock_uts = mocker.patch('tradescope.tradescopebot.TradescopeBot.update_trade_state')

    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order', return_value={'status': 'open'})
    create_mock_trades(fee, is_short)
    trades = Trade.get_trades().all()

    tradescope.handle_insufficient_funds(trades[3])
    ...
    tradescope.handle_insufficient_funds(trade)
    # assert log_has_re(r"Trying to reupdate buy fees for .*", caplog)
...
def test_handle_insufficient_funds(mocker, default_conf_usdt, fee, is_short, caplog):
    ...
    tradescope.handle_insufficient_funds(trades[4])
...
def test_handle_onexchange_order(mocker, default_conf_usdt, limit_order, is_short, caplog):
    ...
    tradescope = get_patched_tradescopebot(mocker, default_conf_usdt)
    mock_uts = mocker.spy(tradescope, 'update_trade_state')
    ...
    Trade.session.add(trade)
    tradescope.handle_onexchange_order(trade)
...
def test_get_valid_price(mocker, default_conf_usdt) -> None:
    ...
    patch_exchange(mocker)
    tradescope = TradescopeBot(default_conf_usdt)
    tradescope.config['custom_price_max_distance_ratio'] = 0.02
    ...
    valid_price_from_string = tradescope.get_valid_price(custom_price_string, proposed_price)
    valid_price_from_badstring = tradescope.get_valid_price(custom_price_badstring, proposed_price)
    valid_price_from_int = tradescope.get_valid_price(custom_price_int, proposed_price)
    valid_price_from_float = tradescope.get_valid_price(custom_price_float, proposed_price)

    valid_price_at_max_alwd = tradescope.get_valid_price(custom_price_over_max_alwd, proposed_price)
    valid_price_at_min_alwd = tradescope.get_valid_price(custom_price_under_min_alwd, proposed_price)
...
def test_update_funding_fees_schedule(mocker, default_conf, trading_mode, calls, time_machine,
    ...
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    mocker.patch('tradescope.tradescopebot.TradescopeBot.update_funding_fees', return_value=True)
    default_conf['trading_mode'] = trading_mode
    default_conf['margin_mode'] = 'isolated'
    tradescope = get_patched_tradescopebot(mocker, default_conf)

    time_machine.move_to(f"{t2} {tzoffset}", tick=False)
    # Check schedule jobs in debugging with tradescope._schedule.jobs
    tradescope._schedule.run_pending()

    assert tradescope.update_funding_fees.call_count == calls
...
def test_update_funding_fees(
    ...
    tradescope.execute_entry('ETH/USDT', 123, is_short=is_short)
    tradescope.execute_entry('LTC/USDT', 2.0, is_short=is_short)
    tradescope.execute_entry('XRP/USDT', 123, is_short=is_short)
...
def test_update_funding_fees_error(mocker, default_conf, caplog):
    ...
    default_conf['margin_mode'] = 'isolated'
    tradescope = get_patched_tradescopebot(mocker, default_conf)
    tradescope.update_funding_fees()
...
def test_position_adjust(mocker, default_conf_usdt, fee) -> None:
    ...
    assert tradescope.execute_entry(pair, stake_amount, trade=trade)
    ...
    assert tradescope.execute_trade_exit(trade=trade, limit=8,
...
def test_position_adjust2(mocker, default_conf_usdt, fee) -> None:
    ...
    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order',
                 MagicMock(return_value=closed_successful_buy_order))
    assert tradescope.execute_entry(pair, amount)
    # Should create an closed trade with an no open order id
    # Order is filled and trade is open
    orders = Order.session.scalars(select(Order)).all()
    assert orders
    ...
    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order',
                 MagicMock(return_value=closed_sell_dca_order_1))
    assert tradescope.execute_trade_exit(trade=trade, limit=ask,
                                        exit_check=ExitCheckTuple(exit_type=ExitType.PARTIAL_EXIT),
...
def test_position_adjust3(mocker, default_conf_usdt, fee, data) -> None:
    ...
    for idx, (order, result) in enumerate(data):
        ...
        if order[0] == 'buy':
            assert tradescope.execute_entry(pair, amount, trade=trade)
        else:
            assert tradescope.execute_trade_exit(
...
def test_process_open_trade_positions_exception(mocker, default_conf_usdt, fee, caplog) -> None:
    ...
    mocker.patch('tradescope.tradescopebot.TradescopeBot.check_and_call_adjust_trade_position',
                 side_effect=DependencyException())

    create_mock_trades(fee)

    tradescope.process_open_trade_positions()
...
def test_check_and_call_adjust_trade_position(mocker, default_conf_usdt, fee, caplog) -> None:
    ...
    create_mock_trades(fee)
    caplog.set_level(logging.DEBUG)
    tradescope.strategy.adjust_trade_position = MagicMock(return_value=(10, 'aaaa'))
    tradescope.process_open_trade_positions()
    ...
    caplog.clear()
    tradescope.strategy.adjust_trade_position = MagicMock(return_value=(-0.0005, 'partial_exit_c'))
    tradescope.process_open_trade_positions()
