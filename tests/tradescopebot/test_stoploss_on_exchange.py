...
def test_handle_stoploss_on_exchange(mocker, default_conf_usdt, fee, caplog, is_short,
    ...
    assert tradescope.handle_stoploss_on_exchange(trade) is False
    assert stoploss.call_count == 1
    ...
    assert tradescope.handle_stoploss_on_exchange(trade) is True
    ...
    trade.is_open = True
    tradescope.handle_stoploss_on_exchange(trade)
...
def test_handle_stoploss_on_exchange_emergency(mocker, default_conf_usdt, fee, is_short,
    ...
    tradescope.config['trailing_stop'] = True
...
def test_handle_stoploss_on_exchange_partial_cancel_here(
    ...
    # Tradescope cancels the stop - but cancel returns a partial filled order.
    stoploss_order_hit = MagicMock(return_value={
        'id': "101",
        'status': 'open',
        'type': 'stop_loss_limit',
...
def test_create_stoploss_order_invalid_order(
    ...
    caplog.clear()
    rpc_mock.reset_mock()
    tradescope.create_stoploss_order(trade, 200)
    assert trade.has_open_sl_orders is False
...
def test_handle_stoploss_on_exchange_trailing(
    ...
    # setting stoploss
    tradescope.strategy.stoploss = 0.05 if is_short else -0.05

    # setting stoploss_on_exchange_interval to 60 seconds
    tradescope.strategy.order_types['stoploss_on_exchange_interval'] = 60
    ...
    stoploss_order_mock.assert_called_once_with(
        amount=30,
        pair='ETH/USDT',
        order_types=tradescope.strategy.order_types,
...
def test_handle_stoploss_on_exchange_trailing_error(
    ...
    tradescope.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
...
def test_stoploss_on_exchange_price_rounding(
    ...
    tradescope.handle_trailing_stoploss_on_exchange(open_trade_usdt, {})
...
def test_handle_stoploss_on_exchange_custom_stop(
    ...
    # setting stoploss
    tradescope.strategy.custom_stoploss = lambda *args, **kwargs: -0.04
    ...
    tradescope.strategy.order_types['stoploss_on_exchange_interval'] = 0
    cancel_order_mock.assert_not_called()
    stoploss_order_mock.assert_not_called()
...
def test_tsl_on_exchange_compatible_with_edge(mocker, edge_conf, fee, limit_order) -> None:
    ...
    # setting stoploss
    tradescope.strategy.stoploss = -0.02
    ...
    tradescope.active_pair_whitelist = tradescope.edge.adjust(tradescope.active_pair_whitelist)
...
def test_execute_trade_exit_sloe_cancel_exception(
    ...
    mocker.patch('tradescope.wallets.Wallets.get_free', MagicMock(return_value=300))
    create_order_mock = MagicMock(side_effect=[
        {'id': '12345554'},
        {'id': '12345555'},
    ])
    ...
    tradescope.execute_trade_exit(trade=trade, limit=1234,
                                 exit_check=ExitCheckTuple(exit_type=ExitType.STOP_LOSS))
    assert create_order_mock.call_count == 2
...
def test_may_execute_trade_exit_after_stoploss_on_exchange_hit(
    ...
    tradescope = TradescopeBot(default_conf_usdt)
    tradescope.strategy.order_types['stoploss_on_exchange'] = True
    patch_get_signal(tradescope, enter_long=not is_short, enter_short=is_short)
