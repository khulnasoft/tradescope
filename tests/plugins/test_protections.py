...
def test_protectionmanager(mocker, default_conf):
    ...
    for handler in tradescope.protections._protection_handlers:
...
def test_stoploss_guard(mocker, default_conf, fee, caplog, is_short):
    ...
    assert not tradescope.protections.global_stop()
    ...
    assert tradescope.protections.global_stop()
    ...
    tradescope.protections.global_stop(end_time)
...
def test_stoploss_guard_perpair(mocker, default_conf, fee, caplog, only_per_pair, only_per_side):
    ...
    message = r"Trading stopped due to .*"
    pair = 'XRP/BTC'
    assert not tradescope.protections.stop_per_pair(pair)
    assert not tradescope.protections.global_stop()
    assert not log_has_re(message, caplog)
    caplog.clear()
    ...
    # 3 Trades closed - but the 2nd has been closed too long ago.
    assert not tradescope.protections.stop_per_pair(pair)
    assert tradescope.protections.global_stop() != only_per_pair
    ...
    tradescope.protections.stop_per_pair(pair)
    assert tradescope.protections.global_stop() != only_per_pair
...
def test_CooldownPeriod(mocker, default_conf, fee, caplog):
    ...
    message = r"Trading stopped due to .*"
    assert not tradescope.protections.global_stop()
    assert not tradescope.protections.stop_per_pair('XRP/BTC')
    ...
    assert not tradescope.protections.global_stop()
    assert tradescope.protections.stop_per_pair('XRP/BTC')
    ...
    assert tradescope.protections.stop_per_pair('ETH/BTC')
...
def test_LowProfitPairs(mocker, default_conf, fee, caplog, only_per_side):
    ...
    Trade.commit()
    assert tradescope.protections.stop_per_pair('XRP/BTC') != only_per_side
    ...
    assert tradescope.protections.global_stop() != only_per_side
    assert tradescope.protections.stop_per_pair('XRP/BTC') != only_per_side
...
def test_protection_manager_desc(mocker, default_conf, protectionconf,
    ...
    short_desc = str(tradescope.protections.short_desc())
