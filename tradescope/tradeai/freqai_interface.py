...
class ITradeaiModel(ABC):
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.tradeai_info: Dict[str, Any] = config["tradeai"]
        self.data_split_parameters: Dict[str, Any] = config.get("tradeai", {}).get(
            "data_split_parameters", {})
        self.model_training_parameters: Dict[str, Any] = config.get("tradeai", {}).get(
            "model_training_parameters", {})
        self.identifier: str = self.tradeai_info.get("identifier", "no_id_provided")
        self.retrain = False
        self.first = True
        self.set_full_path()
        self.save_backtest_models: bool = self.tradeai_info.get("save_backtest_models", True)
        if self.save_backtest_models:
            logger.info('Backtesting module configured to save all models.')

        self.dd = TradeaiDataDrawer(Path(self.full_path), self.config)
    ...
    def assert_config(self, config: Config) -> None:
        ...
        if not config.get("tradeai", {}):
            raise OperationalException("No tradeai parameters found in configuration file.")

    def start(self, dataframe: DataFrame, metadata: dict, strategy: IStrategy) -> DataFrame:
        """
        Entry point to the TradeaiModel from a specific pair, it will train a new model if
        ...
        if self.live:
            ...
            self.dk = TradeaiDataKitchen(self.config, self.live, metadata["pair"])
        ...
        # TradeAI slides the window and sequentially builds the backtesting results before returning
        # the concatenated results for the full backtesting period back to the strategy.
        else:
            self.dk = TradeaiDataKitchen(self.config, self.live, metadata["pair"])
            if not self.config.get("tradeai_backtest_live_models", False):
    ...
    def _start_scanning(self, strategy: IStrategy) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            dk = TradeaiDataKitchen(self.config, self.live, pair)
            (
                retrain,
            ...
            if retrain:
                ...
                if self.tradeai_info.get('write_metrics_to_disk', False):
                    self.dd.save_metric_tracker_to_disk()

    def start_backtesting(
        self, dataframe: DataFrame, metadata: dict, dk: TradeaiDataKitchen, strategy: IStrategy
    ) -> TradeaiDataKitchen:
        """
        ...
        following the training window). TradeAI slides the window and sequentially builds
        ...
        :param dk: TradeaiDataKitchen = Data management/analysis tool associated to present pair only
        :param strategy: Strategy to train on
        :return:
            TradeaiDataKitchen = Data management/analysis tool associated to present pair only
        """
    ...
    def start_live(
        self, dataframe: DataFrame, metadata: dict, strategy: IStrategy, dk: TradeaiDataKitchen
    ) -> TradeaiDataKitchen:
        """
        ...
        dk: TradeaiDataKitchen = Data management/analysis tool associated to present pair only
        :returns:
        dk: TradeaiDataKitchen = Data management/analysis tool associated to present pair only
        """

        if not strategy.process_only_new_candles:
            raise OperationalException("You are trying to use a TradeAI strategy with "
                                       "process_only_new_candles = False. This is not supported "
                                       "by TradeAI, and it is therefore aborting.")
    ...
    def build_strategy_return_arrays(
        self, dataframe: DataFrame, dk: TradeaiDataKitchen, pair: str, trained_timestamp: int
    ) -> None:
        ...
        if self.tradeai_info.get('fit_live_predictions_candles', 0) and self.live:
    ...
    def check_if_feature_list_matches_strategy(
        self, dk: TradeaiDataKitchen
    ) -> None:
        """
        Ensure user is passing the proper feature set if they are reusing an `identifier` pointing
        to a folder holding existing models.
        :param dataframe: DataFrame = strategy provided dataframe
        :param dk: TradeaiDataKitchen = non-persistent data container/analyzer for
        ...
        if dk.training_features_list != feature_list:
            raise OperationalException(
                ...
                "model. In case of --strategy-list, please be aware that TradeAI "
    ...
    def model_exists(self, dk: TradeaiDataKitchen) -> bool:
        """
    ...
    def extract_data_and_train_model(
        ...
        :param dk: TradeaiDataKitchen = non-persistent data container for current coin/loop
    ...
    def set_initial_historic_predictions(
        self, pred_df: DataFrame, dk: TradeaiDataKitchen, pair: str, strat_df: DataFrame
    ) -> None:
        """
        ...
        data. After that, TradeAI will append new real predictions to the
        ...
        :param pred_df: DataFrame = the dataframe containing the predictions coming
            out of a model
        :param dk: TradeaiDataKitchen = object containing methods for data analysis
        ...
        if self.tradeai_info['feature_parameters'].get('DI_threshold', 0) > 0:
    ...
    def fit_live_predictions(self, dk: TradeaiDataKitchen, pair: str) -> None:
        """
        ...
        num_candles = self.tradeai_info.get("fit_live_predictions_candles", 100)
    ...
    def inference_timer(self, do: Literal['start', 'stop'] = 'start', pair: str = ''):
        """
        Timer designed to track the cumulative time spent in TradeAI for one pass through
    ...
    def cache_corr_pairlist_dfs(self, dataframe: DataFrame, dk: TradeaiDataKitchen) -> DataFrame:
        """
    ...
    def track_current_candle(self):
        ...
        equivalent to the latest candle seen by TradeAI. If not, it
    ...
    def ensure_data_exists(self, len_dataframe_backtest: int,
        ...
        """
        if self.config.get("tradeai_backtest_live_models", False) and len_dataframe_backtest == 0:
    ...
    def backtesting_fit_live_predictions(self, dk: TradeaiDataKitchen):
        """
        ...
        """
        fit_live_predictions_candles = self.tradeai_info.get("fit_live_predictions_candles", 0)
    ...
    def start_backtesting_from_historic_predictions(
        self, dataframe: DataFrame, metadata: dict, dk: TradeaiDataKitchen
    ) -> TradeaiDataKitchen:
        """
        :param dataframe: DataFrame = strategy passed dataframe
        :param metadata: Dict = pair metadata
    ...
    # See tradeai/prediction_models/CatboostPredictionModel.py for an example.

    @abstractmethod
    def train(self, unfiltered_df: DataFrame, pair: str,
              dk: TradeaiDataKitchen, **kwargs) -> Any:
        """
    ...
    @abstractmethod
    def fit(self, data_dictionary: Dict[str, Any], dk: TradeaiDataKitchen, **kwargs) -> Any:
        """
        Most regressors use the same function names and arguments e.g. user
        can drop in LGBMRegressor in place of CatBoostRegressor and all data
        management will be properly handled by Tradeai.
    ...
    def data_cleaning_train(self, dk: TradeaiDataKitchen, pair: str):
        """
    ...
    def data_cleaning_predict(self, dk: TradeaiDataKitchen, pair: str):
        """
        throw deprecation warning if this function is called
        """
        logger.warning(f"Your model {self.__class__.__name__} relies on the deprecated"
