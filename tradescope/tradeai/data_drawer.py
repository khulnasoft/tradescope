...
class TradeaiDataDrawer:
    ...
    def __init__(self, full_path: Path, config: Config):
        ...
        self.config = config
        self.tradeai_info = config.get("tradeai", {})
        ...
        self.empty_pair_dict: pair_info = {
                ...
                "data_path": "", "extras": {}}
        self.model_type = self.tradeai_info.get('model_save_type', 'joblib')
        ...
        if exists:
            try:
                ...
                with self.historic_predictions_bkp_path.open("rb") as fp:
                    self.historic_predictions = cloudpickle.load(fp)
                logger.warning('TradeAI successfully loaded the backup historical predictions file.')
    ...
    def set_initial_return_values(self, pair: str,
        ...
        if len(common_dates.index) > 0:
            ...
            logger.warning("No common dates found between new predictions and historic "
                           "predictions. You likely left your TradeAI instance offline "
    ...
    def append_model_predictions(self, pair: str, predictions: DataFrame,
                                 ...
                                 dk: TradeaiDataKitchen, strat_df: DataFrame) -> None:
        """
    ...
    def return_null_values_to_strategy(self, dataframe: DataFrame, dk: TradeaiDataKitchen) -> None:
        """
    ...
    def purge_old_models(self) -> None:
        ...
        num_keep = self.tradeai_info["purge_old_models"]
        if not num_keep:
            return
            ...
            if delete_dict[coin]["num_folders"] > num_keep:
                ...
                for k, v in sorted_dict.items():
                    if deleted >= num_delete:
                        break
                    logger.info(f"Tradeai purging old model file {v}")
                    shutil.rmtree(v)
                    deleted += 1

    def save_metadata(self, dk: TradeaiDataKitchen) -> None:
        """
    ...
    def save_data(self, model: Any, coin: str, dk: TradeaiDataKitchen) -> None:
        """
    ...
    def load_metadata(self, dk: TradeaiDataKitchen) -> None:
        """
    ...
    def load_data(self, coin: str, dk: TradeaiDataKitchen) -> Any:  # noqa: C901
        """
        ...
        if dk.live and coin in self.model_dictionary:
            ...
            MODELCLASS = getattr(mod, self.tradeai_info['rl_config']['model_type'])
    ...
    def update_historic_data(self, strategy: IStrategy, dk: TradeaiDataKitchen) -> None:
        """
        ...
        """
        feat_params = self.tradeai_info["feature_parameters"]
    ...
    def load_all_pair_histories(self, timerange: TimeRange, dk: TradeaiDataKitchen) -> None:
        """
        ...
        for pair in dk.all_pairs:
            if pair not in history_data:
                history_data[pair] = {}
            for tf in self.tradeai_info["feature_parameters"].get("include_timeframes"):
    ...
    def get_base_and_corr_dataframes(
        self, timerange: TimeRange, pair: str, dk: TradeaiDataKitchen
        ...
        with self.history_lock:
            ...
            pairs = self.tradeai_info["feature_parameters"].get(
    ...
    def get_timerange_from_live_historic_predictions(self) -> TimeRange:
        ...
        if not self.historic_predictions_path.is_file():
            raise OperationalException(
                ...
                'to run backtest with the tradeai-backtest-live-models option '
            )
